...
// viewActiveTasks component will render a table that has
// your active tasks and it will allow the user
// to send change the tasks status or delete them
function ViewActiveTasks({ name }) {
  // hooks needed
  ...
  useEffect(() => {
    ...
    fetch(`/get-tasks/${name}/${status}`)
      ...
      .then((data) => {
        if (data.length === 0) {
          serverMessage(`Hi ${name}! You have zero active tasks`);
          setTasks(data.tasks);
          setLoading(false);
        } else {
          setTasks(data.tasks);
      ...
      .catch((err) => {
        setServerMessage("Error fetching active tasks");
      });
  }, []);
  ...
  const currentTasks = tasks.slice(offset, offset + PER_PAGE).map((task) => (
    <tr
      key={task.id}
      className="bg-blue-500 border-b border-blue-700
       text-white font-semibold"
    >
      <td className="px-6 py-4 w-1/6">
        <button onClick={() => handleChangeStatus(task._id, "close")}>
          Close
        </button>
        <span>&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;</span>
  ...
  // Returns this when there is no data
  if (!loading && tasks.length === 0) {
    return (
      <div className="font-bold self-center text-md text-gray-700 mt-2 mx-7">
        You don't have any active tasks currently...
      </div>
    );
  }

  // If there is data it returns this jsx code
  if (tasks.length >= 1) {
    return (
      <div>
...
export default ViewActiveTasks;
